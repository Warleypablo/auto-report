...
function rankMedal(i: number): { label: string; cls: string } {
  if (i === 0) return { label: "🥇", cls: "text-base" };
  if (i === 1) return { label: "🥈", cls: "text-base" };
  if (i === 2) return { label: "🥉", cls: "text-base" };
  return { label: `#${i + 1}`, cls: "font-mono-num text-xs text-[var(--muted)]" };
}
...
function MetaDrawer({ ad, allAds, onClose }: { ad: RankedMetaAd; allAds: RankedMetaAd[]; onClose: () => void }) {
  ...
  const { label: medal } = rankMedal(ad.rank);
    ...
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 flex h-full w-[380px] flex-col bg-[var(--paper)] shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {ad.imagem_url && (
            <div className="mb-5 overflow-hidden rounded-xl border border-[var(--rule-soft)] bg-[var(--paper-deep)]">
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[var(--muted)]">Posição no ranking · Meta Ads</p>
              <p className="text-xs text-[var(--muted)]">{adsComRoas.length} criativos comparados</p>
            </div>
            <span className={`ml-auto rounded-md px-2.5 py-1 text-sm font-semibold ${TIER_TEXT[tier]}`}>
              {fmtRoas(ad.roas)}
            </span>
          </div>
          ...
          {roasVsAvg != null && (
            <div className="mb-4 rounded-xl border border-[var(--rule-soft)] bg-[var(--paper-soft)] px-4 py-3">
              ...
              <p className="mt-0.5 text-[10px] text-[var(--muted)]">Média dos criativos: {avgRoas ? fmtRoas(avgRoas) : "—"}</p>
            </div>
          )}
      ...
      <aside className="fixed right-0 top-0 z-50 flex h-full w-[380px] flex-col bg-[var(--paper)] shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <div className="mb-5 flex items-center gap-3">
            <span className="text-2xl">{medal}</span>
            <div>
              <p className="text-xs text-[var(--muted)]">Posição no ranking · Google Ads</p>
              <p className="text-xs text-[var(--muted)]">{adsComRoas.length} campanhas comparadas</p>
            </div>
            <span className={`ml-auto rounded-md px-2.5 py-1 text-sm font-semibold ${TIER_TEXT[tier]}`}>
              {fmtRoas(ad.roas)}
            </span>
          ...
          {roasVsAvg != null && (
            <div className="mb-4 rounded-xl border border-[var(--rule-soft)] bg-[var(--paper-soft)] px-4 py-3">
              ...
              <p className="mt-0.5 text-[10px] text-[var(--muted)]">Média das campanhas: {avgRoas ? fmtRoas(avgRoas) : "—"}</p>
            </div>
          )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--rule-soft)] bg-[var(--paper-soft)]">
                    <th className={`${TH_L} pl-4 pr-2 w-12`}></th>
                    <th className={`${TH_L} px-3`}>Criativo</th>
                    <th className={`${TH_L} px-3`}>Cliente</th>
                    <th className={`${TH_R} px-3`}>ROAS</th>
                    <th className={`${TH_R} px-3`}>Faturamento</th>
                    <th className={`${TH_R} px-3`}>Investimento</th>
                    <th className={`${TH_R} px-3`}>Conv.</th>
                    <th className={`${TH_R} px-3`}>Leads</th>
                    <th className={`${TH_R} pl-3 pr-4`}>CPA/CPL</th>
                ...
                <tbody className="divide-y divide-[var(--rule-soft)]">
                  {metaAds.map((ad) => {
                    const tier = roasTier(ad.roas);
                    const { label: medal, cls: medalCls } = rankMedal(ad.rank);
                    const isTop = ad.rank < 3;
                    return (
                      <tr
                        key={`${ad.clienteSlug}-${ad.nome}`}
                        onClick={() => setSelectedMeta(ad)}
                        className={`group cursor-pointer transition hover:bg-[var(--paper-deep)] ${isTop ? "bg-[var(--paper-soft)]" : "bg-[var(--paper)]"}`}
                      >
                        <td className="py-3 pl-4 pr-2 text-center">
                          <span className={medalCls}>{medal}</span>
                        </td>
                        <td className="max-w-[200px] px-3 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="relative h-8 w-12 flex-shrink-0 overflow-hidden rounded-md bg-gradient-to-br from-[var(--paper-deep)] to-[var(--paper-soft)]">
                              {ad.imagem_url && (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img
                                  src={ad.imagem_url}
                            ...
                            <span className="block truncate text-xs font-medium text-[var(--ink)]" title={ad.nome}>
                              {ad.nome}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className={`px-3 py-3 text-right font-mono-num text-sm font-semibold ${TIER_TEXT[tier]}`}>
                          {fmtRoas(ad.roas)}
                        </td>
                        <td className="px-3 py-3 text-right font-mono-num text-xs text-[var(--forest)]">
                          {fmtK(ad.faturamento)}
                        </td>
                        <td className="px-3 py-3 text-right font-mono-num text-xs text-[var(--muted)]">
                        ...
                        <td className="pl-3 pr-4 py-3 text-right font-mono-num text-xs text-[var(--muted)]">
                          {ad.cpa != null ? fmtK(ad.cpa) : fmtK(ad.cpl)}
                        </td>
                        <td className="pr-3 py-3 text-[var(--muted)] opacity-0 transition group-hover:opacity-100">
            ...
            <div className="overflow-hidden rounded-xl border border-[var(--rule-soft)]">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--rule-soft)] bg-[var(--paper-soft)]">
                    <th className={`${TH_L} pl-4 pr-2 w-12`}></th>
                    <th className={`${TH_L} px-3`}>Campanha</th>
                    <th className={`${TH_L} px-3`}>Cliente</th>
                    <th className={`${TH_R} px-3`}>ROAS</th>
                    <th className={`${TH_R} px-3`}>Faturamento</th>
                    <th className={`${TH_R} px-3`}>Investimento</th>
                    <th className={`${TH_R} px-3`}>Conv.</th>
                    <th className={`${TH_R} pl-3 pr-4`}>CPA</th>
                    <th className="w-6 pr-3"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--rule-soft)]">
                  {googleAds.map((c) => {
                    ...
                    const { label: medal, cls: medalCls } = rankMedal(c.rank);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="max-w-[280px] px-3 py-3">
                          <span className="block truncate text-xs font-medium text-[var(--ink)]" title={c.nome}>
                            {c.nome}
                          </span>
                        </td>
                        <td className="whitespace-nowrap px-3 py-3 text-xs text-[var(--muted)]">
